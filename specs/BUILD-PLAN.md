...
**Chat-independent reference — staging empty buy FG list (2026-09-08):** Local Vite shows Colchicine etc. because local `finished_goods` was synced from `pharmadb`. Staging `http://43.225.195.200:8082/api/v1/finished-goods` returned `[]` with the new empty-catalogue copy — the **JAR is current**; the snapshot table is empty. `prod` defaults `exhibition.pharma-erp.enabled=false`. Git does not copy local MySQL rows. Host: uncomment `EXHIBITION_PHARMA_ERP_*` in `C:\exhibition-portal-staging\portal.env.ps1`, re-run `install-service.ps1 -Staging` (WinSW XML), restart, then **Staff → Sync finished goods**. `verify-staging.ps1` warns if enabled is not true and reports FG row count.
