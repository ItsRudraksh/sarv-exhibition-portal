...
**Chat-independent reference — public domain reverse proxy (2026-09-11):** Staging stays on **8083**. Marketing [sarvbiolabs.com](https://sarvbiolabs.com/) is **209.42.22.88**; exhibition Java is **43.225.195.200**. Prefer **`welcome.sarvbiolabs.com`** (`A` → `43.225.195.200`, `VITE_BASE=/`, no context-path, IIS on this host). Path `sarvbiolabs.com/exhibit` is on the WordPress host and needs **both** `VITE_BASE=/exhibit/` and `SERVER_SERVLET_CONTEXT_PATH=/exhibit`. **TLS:** WordPress HTTPS does not cover 8083. Terminate HTTPS on IIS 443 for `welcome.sarvbiolabs.com`; Java stays `http://127.0.0.1:8083/`. Direct IP:8083 remains HTTP (upload only for camera). Samples: `deploy/windows/iis/`. Test: `PublicPathPrefixTest`. Runbook: [DEPLOY-WINDOWS.md](DEPLOY-WINDOWS.md) § Public domain / Where HTTPS lives.
