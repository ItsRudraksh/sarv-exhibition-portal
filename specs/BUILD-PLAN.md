...
**Chat-independent reference — Code walkthrough comments (2026-09-11):** KT comments live in `package-info.java`, class Javadoc, and frontend file headers. Start at [CODE-WALKTHROUGH.md](CODE-WALKTHROUGH.md). Comments explain intent and product constraints, not line-by-line narration.

**Chat-independent reference — Other details textbox (2026-09-11):** Checking Other on sell categories, sell product types, or the buy catalogue reveals required **Describe Other**. Unchecking clears the text. Submit rejects Other without details. Flyway V12: `supplier_inquiries.other_category_detail` / `other_product_type_detail`; `purchase_inquiries.other_product` / `other_product_detail`. Buyer Other-only still writes `requirement_text` so staff/export see it. Tests: `InquiryRulesTest`. UI: `OtherDetailsField` on `SupplierDepartmentsScreen`, `SupplierProductTypesScreen`, `BuyerNeedScreen`.

**Chat-independent reference — Product-type selected categories (2026-09-11):** After Continue from categories, the product-type screen keeps those categories **checked** (read-only) and lists product types under each (`groupProductTypesByDepartments`). UI: `SupplierProductTypesScreen`.

**Chat-independent reference — Review Edit return (2026-09-11):** Supplier smart-details and review summary cards expose **Edit**. `startEdit` snapshots the draft (in-memory, not persisted as a field) and opens the matching screen. Footer is **Cancel** (restore snapshot) / **Save changes** (`finishEdit` returns to the originating review or smart-details step). Taxonomy Edit can continue to product types via **Edit product types**. Buyer review uses the same return for need and contact. UI: `ReviewEditFooter`, `useInquiryJourney` `startEdit`/`finishEdit`/`cancelEdit`.
...
**Chat-independent reference — Supplier either-or offering (2026-09-10):** Category and subcategory screens show `capabilityNotes` **above** the checkbox list. Continue if the visitor selects a listed item/Other **or** fills the description. Submit: category signal = departments or `otherCategory` or notes; type signal = product types or `otherProductType` or notes. Tests: `InquiryRulesTest`. UI: `SupplierDepartmentsScreen`, `SupplierProductTypesScreen`.

**Chat-independent reference — Supplier Other + attachments (2026-09-09):** Supplier submit no longer requires a listed department+product-type pair. Categories (departments) **or Other**; free-text `capabilityNotes` when the list does not fit; product types optional when Other/notes cover the offering. Flyway V11: `supplier_inquiries.other_category` / `other_product_type` / `capability_notes`; `file_assets.purpose` adds `INQUIRY_ATTACHMENT`. Both routes: multiple supporting files, 5 MiB each, max 10. `DELETE` draft supporting files (soft `PURGED`). Cards stay 10 MiB. Audit: `FILE_REMOVED` (purpose only). Tests: `InquiryRulesTest`, `FileConsentAuditApiTest`, `FileContentRulesTest`. UI: supplier category/subcategory + Other + notes; attachments on supplier review and buyer need.
...
**Chat-independent reference — Jenkins staging port-in-use (2026-09-10):** Job `exibit-portal-pipeline_poc` commit `9ba0e85` compiled and tested (81 tests) then Deploy failed: `Port 8082 was already in use`. Commit `342ff87` identified the listener as **wachatbot** (`C:\wachatbot\app.jar`, JDK 21), not exhibition-portal. Staging moved to **8083**. Do not kill wachatbot from Jenkins. See [DEPLOY-WINDOWS.md](DEPLOY-WINDOWS.md) § Staging deploy: port collision.
...
**Chat-independent reference — PowerShell `$PID` (2026-09-10):** Commit `14b9ceb` parsed and built (81 tests) then Deploy failed in `install-service.ps1`: `Cannot overwrite variable PID because it is read-only`. `foreach ($pid in $listenPids)` assigns the automatic `$PID`. Loop variable is `$owningPid`.
...
**Chat-independent reference — public domain reverse proxy (2026-09-11):** Staging stays on **8083**. Marketing [sarvbiolabs.com](https://sarvbiolabs.com/) is **209.42.22.88**; exhibition Java is **43.225.195.200**. Prefer **`exhibit.sarvbiolabs.com`** (`A` → `43.225.195.200`, `VITE_BASE=/`, no context-path, IIS on this host). Path `sarvbiolabs.com/exhibit` is on the WordPress host and needs **both** `VITE_BASE=/exhibit/` and `SERVER_SERVLET_CONTEXT_PATH=/exhibit`. **TLS:** WordPress HTTPS does not cover 8083. Terminate HTTPS on IIS 443 for `exhibit.sarvbiolabs.com`; Java stays `http://127.0.0.1:8083/`. Direct IP:8083 remains HTTP (upload only for camera). Samples: `deploy/windows/iis/`. Test: `PublicPathPrefixTest`. Runbook: [DEPLOY-WINDOWS.md](DEPLOY-WINDOWS.md) § Public domain / Where HTTPS lives.
