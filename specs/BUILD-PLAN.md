...
**Chat-independent reference — poc staging folder (2026-09-03):** Job `exibit-portal-pipeline_poc` built JAR `80718e21` with SUCCESS but skipped **Deploy to Staging** (`when { branch 'dev' }` only). `C:\exhibition-portal-staging` was never created; `C:\pharma-erp-staging` exists from an earlier pharma install. Jenkinsfile now deploys staging for **`poc` and `dev`**: create the install dir, copy JAR + `start-portal.ps1`, seed `portal.env.ps1` (8081). If service `exhibition-portal-staging` is missing, the folder still remains and the stage fails with `.\deploy\windows\install-service.ps1 -Staging`. Edit passwords, then rebuild. Staging must not bind port 80.
